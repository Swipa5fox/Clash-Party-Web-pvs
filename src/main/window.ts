...
let mainWindowStubbed = false

// Web UI 模式：向 mainWindow 注入合成窗口。webContents.send 转 WS 广播，
// 其余属性访问返回 no-op 函数，保证窗口类 handler（close/setAlwaysOnTop/
// setTitleBarOverlay 等）不崩（ipc.ts 有 typeof === 'function' 判断，函数即可通过）。
export function setMainWindowStub(broadcast: (channel: string, payload?: unknown) => void): void {
  if (mainWindow) return // GUI 模式已有真实窗口，不覆盖
  mainWindowStubbed = true

  const noop = (): void => {}
  const stubWebContents = new Proxy(
    {},
    {
      get: (_target, prop) =>
        prop === 'send'
          ? (channel: string, ...args: unknown[]): void => broadcast(channel, args[0])
          : noop
    }
  )

  mainWindow = new Proxy(
    {},
    {
      get: (_target, prop) => (prop === 'webContents' ? stubWebContents : noop)
    }
  ) as unknown as BrowserWindow
}

export function isMainWindowStub(): boolean {
  return mainWindowStubbed
}
