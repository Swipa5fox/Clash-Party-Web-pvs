...
// 其它情形——needs-reauth（显式重新登录）、active（刷新）、无 vault（首装/换机）——
// 一律走全新浏览器登录 + 新设备，与 spec §9「reauth = 再走一次 login 流程、新设备」一致。
async function runLogin(id: string): Promise<void> {
  ...
  try {
    ...
    // enroll 已成功但凭据无法持久化时，立即回收设备，
    // 避免用户重试登录不断消耗服务端设备额度。
    try {
      await revoke(target, { deviceId }, net)
    } catch {
      // best-effort
    }
    throw error
  }
  const content = await fetchConfig(target, { deviceId }, net)
  await finishLogin(id, record, content)
}
...
function fetchWithRediscovery(
  ...
  const cred = { deviceId: vault.deviceId }
...
// 启动审计只检查 vault 文件/内存是否存在。
// active 但材料缺失（如 vault 文件被删）→ needs-reauth。
