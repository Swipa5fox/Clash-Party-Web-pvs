...
export async function importThemesFromContents(
  files: {
    name: string
    content: string
  }[]
  ...
  for (const file of files) {
    ...
    if (!file.name.endsWith('.css') || !base || base !== file.name) continue
    await atomicWriteFile(
      path.join(themesDir(), `${new Date().getTime().toString(16)}-${base}`),
      file.content
    )
    imported++
  }
