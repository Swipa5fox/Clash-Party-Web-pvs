...
export const mihomoGroups = async (includeHidden = false): Promise<IMihomoMixedGroup[]> => {
  ...
  // 按名字解析成员。必须**递归**: 组里可以嵌组(本部署: AU·通用 -> AU·自动 -> 节点),
  // 只解析一层的话嵌套子组的 all 仍是 string[](名字), 前端渲染子组成员时会对字符串做
  // `'all' in member` → TypeError, 症状是"子组展不开 + 页面卡死"(2026-09-18 踩到)。
  // ponytail: 深度上限 3 层 —— 本部署是 入口组->子组->节点, 够用且天然防环;
  //           真出现更深嵌套再换成带 visited 的备忘录实现。
  // resolveMembers 返回的组成员里,嵌套组已被展开成 all 为对象数组的 IMihomoMixedGroup;
  // depth 耗尽(>3 层嵌套,本部署不存在)时保持原样。前端两种都要能渲染。
  const resolveMembers = (names: string[], depth: number): (IMihomoProxy | IMihomoMixedGroup)[] =>
    (names || [])
      .map((name) => proxies.proxies[name] || providerProxies[name])
      .filter((proxy): proxy is IMihomoProxy | IMihomoGroup => Boolean(proxy))
      .map((member): IMihomoProxy | IMihomoMixedGroup =>
        isMihomoGroup(member) && depth > 0
          ? { ...member, all: resolveMembers(member.all, depth - 1) }
          : (member as unknown as IMihomoMixedGroup)
      )
