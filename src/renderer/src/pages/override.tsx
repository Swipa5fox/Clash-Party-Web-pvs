...
const Override: React.FC = () => {
  ...
  const openLocalOverride = async (file: File | undefined): Promise<void> => {
    if (!file) return
    try {
    ...
    const handleDrop = async (event: DragEvent): Promise<void> => {
      ...
      if (event.dataTransfer?.files) {
        ...
        if (file.name.endsWith('.js') || file.name.endsWith('.yaml')) {
          try {
            // 浏览器/桌面统一直读文件内容（File.path 已废弃）；读取与写入全部包进 try，避免 unhandled rejection
            const content = await file.text()
            await addOverrideItemRef.current({
              name: file.name,
              type: 'local',
            ...
            toast.error(tRef.current('override.error.importFailed', { error: String(e) }))
          } finally {
            setFileOver(false)
          }
        } else {
