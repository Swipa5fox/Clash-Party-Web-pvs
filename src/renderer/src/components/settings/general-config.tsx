...
const GeneralConfig: React.FC = () => {
  ...
  // web 端跳过开机自启探测（checkAutoRun 为宿主专属）
  const { data: enable = false, mutate: mutateEnable } = useSWR(
    isWeb ? null : 'checkAutoRun',
    checkAutoRun
  )
      ...
      <SettingCard>
        ...
        <SettingItem
          ...
          actions={envType.map((type) => (
            <Button
              ...
              onPress={async () => {
                if (isWeb) {
                  // web 端经 copyEnvText 取回文本，由前端写入浏览器剪贴板
                  try {
        ...
        <SettingItem title={t('settings.useWindowFrame')} divider>
          <Switch
            ...
            isDisabled={isRelaunching || isWeb}
          ...
          actions={
            <>
              ...
              <Button
                ...
                onPress={async () => {
                  if (isWeb) {
                    // web 端走浏览器文件选择 + 内容直传（桌面保留 getFilePath + importThemes）
