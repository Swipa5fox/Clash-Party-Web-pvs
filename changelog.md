...
> 基于 Clash Party v2.0.2（[mihomo-party](https://github.com/mihomo-party-org/mihomo-party)）重建的
> **容器化自托管 + 机场插件网关**版本。
> 本文件同时被 `scripts/updater.mjs`（生成自动更新说明 `latest.yml`）与 `scripts/telegram.mjs`（发布通知）读取，
> 撰写时保持「当前版本变更」的口径。
