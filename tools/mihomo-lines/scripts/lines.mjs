...
function loadCfg() {
  let host = process.env.LINES_HOST, token = process.env.LINES_TOKEN
  if (!host || !token) {
    try {
      const c = JSON.parse(readFileSync(new URL('./lines.config.json', import.meta.url), 'utf8'))
      host = host || c.host; token = token || c.token
    } catch {}
  }
  if (!host || !token) {
    console.error('[lines][FAIL] 缺配置: 设 LINES_HOST/LINES_TOKEN 环境变量, 或在脚本同目录放 lines.config.json: {"host":"<ip>","token":"<web-token>"}')
    process.exit(1)
  }
  return { host, token }
}
...
const die = (m) => { console.error('[lines][FAIL] ' + m); process.exit(1) }
...
async function bridge() {
  ...
  await new Promise((res, rej) => {
    ...
    ws.addEventListener('message', (e) => { JSON.parse(e.data).type === 'hello' && ok() })
    ws.addEventListener('error', () => { clearTimeout(t); rej(new Error('WS 连接失败(token/3999?)')) })
  })
  const call = (ch, ...args) => new Promise((resolve, reject) => {
...
function overrideSrc(prefix, base, reStr, forceDomains, flat = false) {
  const glob = base + 1, q = JSON.stringify
  const POOL = `${prefix}·通用[${base}]`, GLOB = `${prefix}·全局[${glob}]`
  const AUTO = `${prefix}·自动`, FB = `${prefix}·故障`, MAN = `${prefix}·手动`
  const mix = `${prefix.toLowerCase()}-mix`, glb = `${prefix.toLowerCase()}-glb`
  const forced = (forceDomains || []).map(d => `AND,((IN-NAME,${mix}),(DOMAIN-SUFFIX,${d})),${POOL}`)
  // 生成源拼装: 只有嵌套模式生成子组;平铺模式 sub 为空数组,入口组直接挂节点。
  const subDecl = flat ? 'const sub = []' : `const sub = ordered.length ? [
...
async function discover() {
  ...
  for (const [name, g] of Object.entries(data.proxies)) {
    ...
    if (kind === '通用') { L.base = +port; L.mix = name; L.mixNow = g.now }
    else { L.globPort = +port; L.glob = name; L.globNow = g.now }
  }
  return [...lines.values()].filter((l) => l.base || l.globPort)
}

const doorOpen = (port) => new Promise((r) => {
...
function proxyGet(port, urlStr, tmo) {
  return new Promise((resolve) => {
    ...
    const req = http.request({ host: HOST, port, method: 'GET', path: urlStr, headers: { Host: u.host, 'User-Agent': 'curl/8.5.0' } }, (res) => {
      let d = ''
      res.on('data', (c) => (d += c))
      res.on('end', () => resolve(d))
    })
    req.on('error', () => resolve(null))
    req.setTimeout(tmo ?? 9000, () => { req.destroy(); resolve(null) })
    req.end()
  })
}
...
function proxyHttpsStatus(port, hostname, tmo = 12000) {
  return new Promise((resolve) => {
    const req = http.request({ host: HOST, port, method: 'CONNECT', path: hostname + ':443', headers: { Host: hostname + ':443', 'User-Agent': 'curl/8.5.0' } })
    const fin = (v) => { try { req.destroy() } catch {}; resolve(v) }
    req.setTimeout(tmo, () => fin(0))
    req.on('connect', (res, socket) => {
      if (res.statusCode !== 200) return fin(res.statusCode)
      const tls = tlsLib.connect({ socket, servername: hostname, rejectUnauthorized: false }, () => {
      ...
      tls.on('data', (d) => {
        ...
        if (m) { try { tls.destroy() } catch {}; resolve(+m[1]) }
      })
      tls.on('error', () => resolve(0))
      tls.setTimeout(tmo, () => { try { tls.destroy() } catch {}; resolve(0) })
    })
...
if (cmd === 'list') {
  ...
  for (const l of lines.sort((x, y) => (x.base || 0) - (y.base || 0))) {
    ...
    if (l.globPort) log(`${String(l.globPort).padEnd(7)}${(l.glob || '').padEnd(24)}${l.globNow || '-'}`)
  }
  process.exit(0)
}
...
if (cmd === 'add') {
  ...
  if (!prefix || !baseStr || !reStr) die('参数: add <PREFIX> <base端口> <节点正则> [强制走池域名,逗号分隔]')
  const base = parseInt(baseStr, 10)
  if (!/^\d+$/.test(baseStr) || base < 1024) die('base 端口非法')
  const forceDomains = (domainsStr || '').split(',').map((d) => d.trim()).filter(Boolean)
  ...
  log(`正则命中 ${hit.length} 个节点${hit.length ? ': ' + hit.slice(0, 3).join(' / ') + (hit.length > 3 ? ' ...' : '') : ''}`)
  if (hit.length < 1) die('订阅里没有匹配节点,先核正则')
  const { ws, call } = await bridge().catch((e) => die(e.message))
  const id = `${prefix.toLowerCase()}-two-lines`
  try { await call('removeOverrideItem', id) } catch {}
  await call('addOverrideItem', { id, name: `${prefix} 双口 通用${base}/全局${base + 1}${flat ? '(平铺)' : ''}`, type: 'local', ext: 'js', global: true, file: overrideSrc(prefix, base, reStr, forceDomains, flat) })
  await call('restartCore')
  log(`覆写已写(${flat ? '平铺: 入口组直接挂节点' : '嵌套: 入口组挂 自动/故障/手动 子组'}): ${prefix}·通用[${base}](分流) + ${prefix}·全局[${base + 1}](全局), 内核已重启`)
  if (forceDomains.length) log(`强制走池域名: ${forceDomains.join(', ')}(规则已插到最前)`)
  ws.close()
  const ok1 = await doorOpen(base), ok2 = await doorOpen(base + 1)
  if (ok1 && ok2) { log('端口门已开, 线路可用'); process.exit(0) }
  log(`门未开(${base}:${ok1 ? '开' : '关'} ${base + 1}:${ok2 ? '开' : '关'}) — 服务器上给 docker-compose.override.yml 的 ports 追加:\n      - '${base}:${base}'\n      - '${base + 1}:${base + 1}'\n  然后 docker compose up -d party`)
  process.exit(0)
}
...
if (cmd === 'push') {
  ...
  if (ext === 'js' && !/function\s+main\s*\(/.test(src)) die(`不是合法覆写(缺 function main): ${file}`)
  const { ws, call } = await bridge().catch((e) => die(e.message))
  ...
  if (ext === 'js') log(`核对执行结果: docker exec <party容器> cat /data/.config/mihomo-party-dev/override/${id}.log`)
...
if (cmd === 'remove') {
  ...
  const { ws, call } = await bridge().catch((e) => die(e.message))
  try { await call('removeOverrideItem', `${prefix.toLowerCase()}-two-lines`) } catch { die('覆写不存在') }
  await call('restartCore')
...
if (cmd === 'switch') {
  ...
  await rest('PUT', `/proxies/${encodeURIComponent(group)}`, { name: node }).catch((e) => die(e.message))
  log(`已切换 ${group} -> ${node}`)
  process.exit(0)
}
...
if (cmd === 'verify') {
  ...
  for (const l of lines) {
    ...
    for (const [port, name] of [[l.base, l.mix], [l.globPort, l.glob]]) {
      if (!port) continue
      const foreign = ((await proxyGet(port, 'http://ip-api.com/line/?fields=countryCode')) || '?').trim()
      const cnip = await cnIp(port)
      const cnOut = cnip ? (cnip === ref ? '走了代理!' : '直连(' + cnip + ')') : '?'
      // 小红书只对全局口有意义(分流口命中国内直连,测的是家宽)
      const xhs = port === l.globPort
        ? await (async () => { const c = await proxyHttpsStatus(port, 'www.xiaohongshu.com'); return c ? c + (c < 400 ? '(过)' : '') : 'TLS被掐' })()
        : '-'
      log(`${String(port).padEnd(7)}${(name || '').padEnd(24)}${foreign.padEnd(9)} ${(cnOut || '?').padEnd(20)} ${xhs}`)
    }
  }
  process.exit(0)
}
...
if (cmd === 'trace') {
  ...
  const t0 = Date.now()
  let socket
  const conn = http.request({ host: HOST, port, method: 'CONNECT', path: hostname + ':443', headers: { 'User-Agent': 'curl/8.5.0' } })
  conn.on('connect', (res, s) => { socket = s })
  conn.on('error', () => {})
  conn.end()
  await new Promise((r) => setTimeout(r, 2500))
  const data = await rest('GET', '/connections').catch(die)
  socket?.destroy()
  const hit = (data.connections || []).filter((c) => (c.metadata.host || '').includes(hostname) && c.start * 1000 >= t0 - 2000)
  if (!hit.length) { log('未捕获到连接(可能瞬间完成),再跑一次'); process.exit(0) }
